...
Der Spieler erstellt durch die Auswahl eines Ichs des Chronologen eine Zeitlinie, auf welcher sich dieses Ich zeitlich bewegen wird. Dazu wird zunächst eine Hauptwurzel des Baumes benötigt, welcher im folgenden Codeausschnitt \ref{sec:old_graphController} zu sehen ist:
...
Der $GraphController$ initialisiert zuerst seine eigenen Actions mit seinen Msthoden, ehe er den $TimeController$ und die $TimeStability$ initialisiert. 
...
Die eingezeichnete Schleife, die ab dem Aktivitätsknoten \say{Control of Character} beginnt, wird aufgedröselt in den Kapiteln \ref{sec:components}, \ref{sec:core_algo_split_merge}, \ref{sec:merge_prototype}, \ref{sec:split_prototype} und \ref{sec:interactions_reproduction} erklärt.
...
Damit diese Klasse auf die Actions des $GraphControllers$ regiert, besitzt sie wie in der folgenden Abbildung \ref{fig:recordable-cs} entsprechende Methoden.
...
In diesem Zusammenhang gibt es zusätzlich $AudioSourceController$ für den Sprachdialog des Chronologen und anderen Sprachdialogteilnehmern (vgl. Abbildung \ref{fig:audio-source-controller}). 
...
Die Durchführung der Nutzertests erfolgte nach der Methodik des \say{\ac{CTA}}. Dabei sollten die Testpersonen ihre Gedanken zum Prototyp aussprechen und ggf. ihr Feedback einbringen (vgl. \cite{usabilityde_gmbh__co_kg_concurrent_2023}, 2023). Die Kenntnisse des \say{\ac{CTA}} werden ebenfalls wie der Fragebogen gleichermaßen ausgewertet und im Folgenden \say{Kapitel \ref{sec:test-result}: \nameref{sec:test-result}} vorgestellt.
...
Im Prototyp kann der Spieler über die Tasten \say{A} und \say{D} das Auswahlrad drehen, um ein Chronologen-Ich auszuwählen. Für die Zukunft wäre es besser, wenn der Spieler auf die Zahnradspitze des gewünschten Ichs klicken kann, welcher dadurch ausgewählt wird. Der \say{RayCast-Input} hat allerdings ein Problem, welches zunächst angepasst werden muss.
...
Dabei war das zielgerichtete \say{Trial and Error }Prinzip beim Lösen der Rätsel einen Höhepunkt und erforderte logisches Denken und Geduld. 
